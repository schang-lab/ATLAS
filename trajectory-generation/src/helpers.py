...
def normalize_prediction_type(prediction_type: str) -> str:
    """Normalize user-specified prediction type to an internal canonical value."""

    if prediction_type is None:
        return "epsilon"

    normalized = prediction_type.strip().lower()

    if normalized in {"eps", "epsilon"}:
        return "epsilon"
    if normalized in {"x", "x0", "x_start", "xstart", "sample", "x-start"}:
        return "x_start"
    if normalized in {"v", "velocity", "v_prediction", "v-prediction", "v-predict"}:
        return "v"

    raise ValueError(f"Unsupported prediction_type '{prediction_type}'. Expected 'epsilon', 'x0', or 'v'.")
