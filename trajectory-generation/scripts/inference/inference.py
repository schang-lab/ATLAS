#!/usr/bin/env python3
"""
Infer-specific DiT-only inference:
attrs[:, 0:4] = [work_lat, work_lon, home_lat, home_lon]
...
# Add the parent directory to Python path to find src and lib modules
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PARENT_DIR = os.path.dirname(os.path.dirname(SCRIPT_DIR))
sys.path.insert(0, PARENT_DIR)

from src.helpers import normalize_prediction_type  
from lib.infer_run import run_inference  
...
def parse_args():
    ...
    # Manual override for attrs used for generation (also used to inject home/work token coords)
    ...
    # Optional: baseline-only demo assignment (for evaluation grouping).
    parser.add_argument("--assign_random_demo", action="store_true", default=False,
                        help="Assign (age_bin, gender_id) to sampled_attributes.npy for baseline evaluation.")
    parser.add_argument("--demo_source_attr_with_demo_npy", type=str, default=None,
                        help="Optional path to real all_attr_results_with_demo.npy to sample demo pairs from. "
                             "Defaults to <test_data_dir>/controlled/<split>/all_attr_results_with_demo.npy.")
    parser.add_argument("--demo_only_for_unconditional", action=argparse.BooleanOptionalAction, default=True,
                        help="If set (default), only assign demos for unconditional rows (uncontrolled or unified-uncontrolled part).")
    ...
    parser.add_argument("--penalize_poi_special_tokens", type=float, default=0.0,
                        help="Penalty (logit subtraction) for POI_HOME, POI_WORK, and POI_OTHER tokens (0.0 = no penalty)")
    parser.add_argument("--penalize_poi_home", type=float, default=0.0,
                        help="Penalty (logit subtraction) for POI_HOME token only (0.0 = no penalty)")
    parser.add_argument("--penalize_poi_work", type=float, default=0.0,
                        help="Penalty (logit subtraction) for POI_WORK token only (0.0 = no penalty)")
    parser.add_argument("--penalize_poi_other", type=float, default=0.0,
                        help="Penalty (logit subtraction) for POI_OTHER token only (0.0 = no penalty)")
    parser.add_argument("--forbid_poi_other_only", action="store_true", default=False,
                        help="Only forbid POI_OTHER token (allow POI_HOME and POI_WORK)")
    parser.add_argument("--penalize_poi_home_work", type=float, default=0.0,
                        help="Penalty (logit subtraction) for POI_HOME and POI_WORK only (0.0 = no penalty, useful for reducing but not eliminating)")

    parser.add_argument("--output_dir", type=str, default=None)
    parser.add_argument("--output_prefix", type=str, default="dit_infer_results")
